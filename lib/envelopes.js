...
export async function verifyEnvelopedCredential({
  config, envelopedCredential, checks, presentationOptions
} = {}) {
  let format;
  try {
    ...
    if(format.typeAndSubType === 'application/jwt') {
      ...
      result = await mdl.verifyEnvelopedCredential({
        config, contents, format, checks, ...presentationOptions
      });
    } else {
...
export async function verifyEnvelopedPresentation({
  ...
  try {
    ...
    } else if(format.typeAndSubType === 'application/mdoc' ||
      // deprecated
      format.typeAndSubType === 'application/mdl-vp-token') {
