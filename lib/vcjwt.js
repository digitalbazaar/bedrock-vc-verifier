...
export async function verifyEnvelopedCredential({jwt} = {}) {
  try {
    ...
    if(verified) {
      ...
      // FIXME: if VC has any proofs, use `di.js` to check VC
...
export async function verifyEnvelopedPresentation({jwt} = {}) {
  try {
    ...
    if(verified) {
      ...
      //throw new BedrockError()...
      // FIXME: verify each VC (which can be DI or enveloped, can't cyclicly
      // include `verify.js` so will have to repeat some code to differentiate)
      // FIXME: add a note here that VC-JWT with 1.1 VP cannot use DI proofs
      // and return false for verified with an error
      // FIXME: otherwise, if VP has any proofs, use `di.js` to check
    }
