  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        response = _createResponse({options, credential, result, checks});
      } catch(e) {
        response = _createResponse({options, error: e});
      }
...
function _createResponse({
  options, credential, result, challengeUses, error, checks
}) {
  // FIXME: return these additional fields:
  /*
  {
    "verified": false,
    ...
    // FIXME: return only if `options.returnProblemDetails`
    "warnings": [ProblemDetails],
    "errors": [ProblemDetails]
    // FIXME: return only if `options.returnCredential`
    "credential": <credential w/o proof>
  }
  */
