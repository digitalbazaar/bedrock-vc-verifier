...
import {verifyEnvelopedCredential} from './envelopes.js';
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        // FIXME: call unified `verify` which will internally handle different
        // securing mechanisms
        let result;
        if(credential?.type === 'EnvelopedVerifiableCredential') {
          result = await verifyEnvelopedCredential({
            envelopedCredential: credential
          });
        } else {
