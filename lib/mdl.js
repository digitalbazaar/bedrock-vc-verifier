...
async function _verifyDeviceResponse({
  ...
  try {
    ...
    */
    ...
    // ensure `mdoc` has one document and its proper `type` is `MDOC_TYPE_MDL`
    // but it can be `MDL_NAMESPACE` for compatibility with `@auth/mdl`
    if(!(mdoc.documents?.length === 1 &&
      mdoc.documents[0].docType === MDOC_TYPE_MDL ||
      mdoc.documents[0].docType === MDL_NAMESPACE)) {
      throw new BedrockError(
