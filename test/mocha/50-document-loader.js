...
describe('document loader option', () => {
  ...
  beforeEach(async () => {
    ...
    // create verifier instance w/document loader option
    const configOptions = {
      verifyOptions: {
        // disable loading remote contexts
        documentLoader: {
  ...
  describe('/credentials/verify', () => {
    ...
    it('does not verify a credential with an unknown context', async () => {
      ...
      error.data.verified.should.equal(false);
      error.data.error.name.should.equal('VerificationError');
      error.data.error.errors[0].message.should.include('Dereferencing');
    });
  });
  ...
  describe('/presentations/verify', () => {
    ...
    it('does not verify a presentation with a credential with an unknown ' +
      'context', async () => {
      // get signing key
      const {methodFor} = await didKeyDriver.generate();
      const signingKey = methodFor({purpose: 'assertionMethod'});
      ...
      e.should.be.an('object');
      should.exist(e.name);
      e.message.should.include('Dereferencing');
    });
  });
});
